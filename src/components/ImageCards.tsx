...
interface IImageCardsProps {
    ...
    image: string
    hoverImage: string
    hoveredCardIndex: hoverType,
    value: number
}

export function ImageCards({ link, image, hoverImage, hoveredCardIndex, value }: IImageCardsProps) {
    return (
        <a href={link}>
            ...
            <div className="border-solid border-2 shadow-2xl w-32 h-32 translate-x-1.5 translate-y-1.5 md:w-60 md:h-60">
                <img src={hoveredCardIndex.hover && (hoveredCardIndex.index === value) ? hoverImage : image} alt="Product Image" />
            </div>
