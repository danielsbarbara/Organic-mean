...
export function DropFilter({ setFilter }: DownFilter) {
  function handleChange(e: any, value: string) {
    if (e) {
      setFilter((prev: any) => ({
        ...prev,
        collection: [...prev.collection, value],
      }));
    } else {
      setFilter((prev: any) => ({
        ...prev,
        collection: prev.collection.filter((el: string) => el !== value),
      }));
    }
  }
      ...
      <div className="flex gap-2 md:gap-7 w-full ml-2">
        <div className="flex flex-col-reverse justify-end items-center gap-2 md:flex-row-reverse">
          <input
            ...
            onChange={(e) => handleChange(e.target.checked, "Anéis")}
          />
          <DropFilterItem
            description={toLocalTranslation("drop_filter_item_1")}
          />
        </div>
        <div className="flex flex-col-reverse justify-end gap-2 md:flex-row-reverse items-center">
          <input
            type="checkbox"
            className="cursor-pointer hover:scale-110 transition duration-300 "
            onChange={(e) => handleChange(e.target.checked, "Pulseiras")}
          />
          <DropFilterItem
            description={toLocalTranslation("drop_filter_item_2")}
          />
        </div>
        <div className="flex flex-col-reverse justify-end gap-2 md:flex-row-reverse items-center">
          <input
            type="checkbox"
            className="cursor-pointer hover:scale-110 transition duration-300 "
            onChange={(e) => handleChange(e.target.checked, "Brincos")}
          />
          <DropFilterItem
            description={toLocalTranslation("drop_filter_item_3")}
          />
        </div>
        <div className="flex flex-col-reverse justify-end gap-2 md:flex-row-reverse items-center">
          <input
            type="checkbox"
            className="cursor-pointer hover:scale-110 transition duration-300 "
            onChange={(e) => handleChange(e.target.checked, "Pendentes")}
          />
          <DropFilterItem
            description={toLocalTranslation("drop_filter_item_4")}
          />
        </div>
      </div>
    </div>
